...
!  Purpose: Evaluate TOPR detection limits and run-length performance.
!==============================================================
module performance_evaluation
...
!  Purpose: Initialize TOPR and SampleGen static data.
!==============================================================
subroutine Init_Performance_Evaluation( Nodesset, B0_all, B1_all, CovMat)
    ...
    ! 2. Initialize TOPR Data
    ! TOPR relies on global params (miu, delta) and internal CUSUM arrays.
    ! It does not strictly need the neighbor matrices for inference like SASAM/CDS,
    ! but we initialize its internal memory here.
    call Init_TOPR_Data()
...
subroutine TOPR_EstimateLimitRange(noise_type, back_index, &
    ...
    if (is_master) then
        ...
        print *, "==== Auto-Estimating Limit Range (TOPR) ===="
    end if
    ...
    if (is_master) then
        ...
        do
            ...
            do j = 1, Time_window
                ...
                ! ADAPTATION: TOPR does not take Limit as input
                call TOPR(OnlineSample, sampling_index, charting_statistic)
            end do                
    ...
    if (is_master) then
        ...
        target_idx_L = int(dble(IC_TestRuns) * (1.0d0*IcArl/(IcArl-50.0d0)))
        target_idx_L = max(1, min(IC_TestRuns, target_idx_L))
        
        target_idx_U = int(dble(IC_TestRuns) * (1.0d0*IcArl/(IcArl+50.0d0)))
...
end subroutine TOPR_EstimateLimitRange

!==============================================================
!  Subroutine: TOPR_limitSearch
!  Cumulative simulation CPU time is recorded.
!==============================================================
subroutine TOPR_limitSearch(noise_type, back_index, &
    ...
    if (is_master) then
        ...
        print *, "==== Begin TOPR_limitSearch (Cumulative Timing) ===="
      ...
      if (is_master) then        
            if (abs(global_arl - IcArl) < Icstd) then
                ...
                if (.not. present(fid)) then
                    print '(A,F12.6,A,F12.6,A,F12.6)', " TOPR Search: [", LimitL, ",", LimitR, "] Try:", Limit
                end if
            end if
        end if
...
end subroutine TOPR_limitSearch

!==============================================================
!  Subroutine: TOPR_OCARL
!  Returns out-of-control, exploration, and exploitation statistics.
!==============================================================
subroutine TOPR_OCARL(noise_type, back_index, anomaly_type, &
    ...
    if (is_master) then
        ...
        all_Exp = 0.0d0
        all_Exploit = 0.0d0
        print *, "==== Begin TOPR_OCARL_Extended ===="
    end if
    ...
    if (is_master) then
        ...
        do while (active_workers > 0)
            ...
            if (mod(recv_count, print_step) == 0) then
                print '(A,I0,A,I0,A,F10.4)', " TOPR OCARL Progress: ", recv_count, "/", simu, &
...
!==============================================================
!  Subroutine: TOPR_ICPerformance
!==============================================================
subroutine TOPR_ICPerformance(noise_type, back_index, Limit, falserate)
    implicit none
    ...
    if (is_master) then
        print *, "==== Begin TOPR_ICPerformance_dynamic ===="
        global_falsecount = 0
    end if
...
end subroutine TOPR_ICPerformance
