program main_driver
    ...
    case ("bspline")
        call Run_TSSPR_Bspline_Test(results_root)
    case ("kernel-circle")
        call Run_TSSPR_Kernel_CircleConv_Test(results_root)
    case default
        if (my_rank == 0) write(*, '(A)') "Unknown TSSPR experiment: " // trim(experiment)
    end select
