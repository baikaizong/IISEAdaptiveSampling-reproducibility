program main_driver
    ...
    if (my_rank == 0) then
        write(*, '(A)') "POS evaluation uses POS_mod with calibrated in-control charting statistics."
    end if
    select case (trim(experiment))
    case ("bspline")
        call Run_POS_Bspline_Test(results_root)
    case ("kernel-circle")
        call Run_POS_Kernel_CircleConv_Test(results_root)
    case default
        if (my_rank == 0) write(*, '(A)') "Unknown POS experiment: " // trim(experiment)
    end select
